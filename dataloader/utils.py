...
def create_logger(conf, phase='train'):
    root_output_dir = Path(conf.OUTPUT_DIR)
    # set up logger
    if not root_output_dir.exists():
        print('creating {}'.format(root_output_dir))
        root_output_dir.mkdir()

    dataset = conf.TRAIN_DATASET
    model = conf.backbone+'_'+conf.metric
    # model += '-' + cfg.TRAIN.MODE
    # cfg_name = os.path.basename(cfg_name).split('.')[0]

    pretrained = 1 if conf.restore else 0
    flag = 'pattern_{}-weight_{}-lr_{}-optim_{}-pretrained_{}'.format(
        conf.S, conf.WEIGHT_PRED , conf.lr, conf.optimizer, pretrained)
    # cfg_name += flag

    final_output_dir = root_output_dir / dataset / model / flag
